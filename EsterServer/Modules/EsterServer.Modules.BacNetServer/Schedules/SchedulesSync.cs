...
namespace EsterServer.Modules.BacNetServer.Schedules
{
    public partial class SchedulesService
    {
			...
			var schedule = new ScheduleClass
				               {
					               ...
					               ControlledObjects = dbSchedule.SchedulesControlledObjects.Select(c => c.Address).ToList()
				               };
		...
		private Schedule ConvertScheduleClassToDbSchedule(ScheduleClass schedule)
		{
			...
			{
				dbSchedule.SchedulesControlledObjects.Add(new SchedulesControlledObject { Address = controlledObject });
			}
        ...
        {
            var masterDevice = _network.OnlineSubscribedDevices.FirstOrDefault(d => d.Id == _masterController);
        ...
        private void EditScheduleOnControllers(ScheduleClass dbSchedule)
        {
            ...
            var schdeuleDevices = _network.SubscribedDevices.Where(d => d.ObjectList.Contains("SCH" + dbSchedule.Id));
        ...
        private void DeleteScheduleFromControllers(ScheduleClass schedule)
        {
            ...
            var controllersWithSchedule =
                _network.SubscribedDevices.Where(d => d.ObjectList.Contains("SCH" + schedule.Id));
        ...
        {
            var tmpSchedule = new ScheduleClass
	                              {Id = sch.Id, Name = sch.Name, Days = sch.Days, ControlledObjects = new List<string>()};
